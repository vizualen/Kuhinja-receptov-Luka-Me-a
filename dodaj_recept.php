...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_FILES["slika"]) && $_FILES["slika"]["name"] != "") {
        ...
        mysqli_query($povezava, "INSERT INTO slike (ime, url, recepti_id) VALUES ('$ime', '$imeSlikeDb', $receptId)");
    }
    ...
    for ($i = 0; $i < count($_POST["kolicina"]); $i++) {
        ...
        $kolicina = mysqli_real_escape_string($povezava, $kolicina);
        $enota = mysqli_real_escape_string($povezava, $enota);
        $sestavinaIme = mysqli_real_escape_string($povezava, $sestavinaIme);
